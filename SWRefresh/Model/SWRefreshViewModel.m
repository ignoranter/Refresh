...
- (void)beginRefreshing:(BOOL)animated source:(id)source{
    ...
    if (animated) {
        self.animating = YES;
        [UIView animateWithDuration:self.beginRefreshingAnimationDuration
                              delay:0
...
- (void)endRefreshing:(BOOL)animated {
    return [self endRefreshingWithState:SWRefreshStateIdle animated:animated reason:SWRefreshEndRefreshSuccessToken];
}
...
- (void)endRefreshingWithState:(SWRefreshState)state animated:(BOOL)animated reason:(id)reason {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, 0.01* NSEC_PER_SEC), dispatch_get_main_queue(), ^{
        ...
        if (animated) {
            ...
            [UIView animateWithDuration:self.endRefreshingAnimationDuration
                                  delay:0
                                options:UIViewAnimationOptionBeginFromCurrentState
                             animations:block
...
#define kSWAnimatingKey @"__animating"
- (BOOL)isAnimating {
    return [_userInfo[kSWAnimatingKey] boolValue];
}

- (void)setAnimating:(BOOL)animating {
    if (animating) {
        _userInfo[kSWAnimatingKey] = @YES;
    } else {
        [_userInfo removeObjectForKey:kSWAnimatingKey];
    }
}

#define kSWBeginRefreshSourceKey @"__BeginRefreshSource"
- (id)beginRefreshingSource {
    return _userInfo[kSWBeginRefreshSourceKey];
}

- (void)setBeginRefreshingSource:(id)beginRefreshingSource {
    if (nil == beginRefreshingSource) {
        [_userInfo removeObjectForKey:kSWBeginRefreshSourceKey];
    } else {
        _userInfo[kSWBeginRefreshSourceKey] = beginRefreshingSource;
    }
}

#define kSWBeginRefreshingAnimationDurationKey @"__beginRefreshAnimationDuration"
- (NSTimeInterval)beginRefreshingAnimationDuration {
    id duration = _userInfo[kSWBeginRefreshingAnimationDurationKey];
...
- (void)setBeginRefreshingAnimationDuration:(NSTimeInterval)beginRefreshingAnimationDuration {
    if (beginRefreshingAnimationDuration == 0) {
        [_userInfo removeObjectForKey:kSWBeginRefreshingAnimationDurationKey];
    } else {
        _userInfo[kSWBeginRefreshingAnimationDurationKey] = @(beginRefreshingAnimationDuration);
    }
}

#define kSWEndRefreshReasonKey @"__endRefreshReason"
- (id)endRefreshingReason {
    return _userInfo[kSWEndRefreshReasonKey];
}

- (void)setEndRefreshingReason:(id)endRefreshingReason {
    if (nil == endRefreshingReason) {
        [_userInfo removeObjectForKey:kSWEndRefreshReasonKey];
    } else {
        _userInfo[kSWEndRefreshReasonKey] = endRefreshingReason;
    }
}

#define kSWEndRefreshingAnimationDurationKey @"__endRefreshAnimationDuration"
- (NSTimeInterval)endRefreshingAnimationDuration {
    id duration = _userInfo[kSWEndRefreshingAnimationDurationKey];
    if (duration) { return [duration doubleValue]; }
    return 0.25; // default value
}
...
- (void)setEndRefreshingAnimationDuration:(NSTimeInterval)endRefreshingAnimationDuration {
    if (endRefreshingAnimationDuration == 0) {
        [_userInfo removeObjectForKey:kSWEndRefreshingAnimationDurationKey];
    } else {
        _userInfo[kSWEndRefreshingAnimationDurationKey] = @(endRefreshingAnimationDuration);
    }
}
