...
@synthesize sourceViewModel = _sourceViewModel;

- (void)dealloc {
    _sourceViewModel = nil;
}
...
+ (id<SWRefreshHeaderController>)headerWithRefreshingBlock:(SWRefreshingBlock)block {
    ...
    return [[self defaultHeaderControllerClass] newWithHeaderView:view model:(id)view.sourceViewModel];
}
...
+ (id<SWRefreshFooterController>)footerWithRefreshingBlock:(SWRefreshingBlock)block {
    ...
    return [[self defaultFooterControllerClass] newWithFooterView:view model:(id)view.sourceViewModel];
}
...
- (void)bindSourceViewModel:(SWRefreshViewModel *)sourceViewModel {
    [sourceViewModel addObserver:self
                      forKeyPath:@"state"
                         options:NSKeyValueObservingOptionOld | NSKeyValueObservingOptionNew
                         context:@"state"];
    [sourceViewModel addObserver:self
                      forKeyPath:@"pullingPercent"
                         options:NSKeyValueObservingOptionOld | NSKeyValueObservingOptionNew
                         context:@"pullingPercent"];
}
...
- (void)observeValueForKeyPath:(nullable NSString *)keyPath
    ...
    if (context == @"state") {
        ...
        if (old != new) {
            [self changeFromState:old to:new];
        }
